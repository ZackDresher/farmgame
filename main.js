...
const IMAGES = {
	account: 'https://i.imgur.com/LfeQv8q.png',
...
class Wheat extends Item {
  ...
  // depending on how many bushels are left
	get info() {
		if (this.quantity < 2) {
			return 'This account is almost gone!'
		} else if (this.quantity < 3) {
			return 'This data is running low'
		} else {
			return 'This is some nice data'
		}
	}
  ...
  // When a Wheat is clicked on...
    ...
    // let the player know
		if (this.quantity <= 0) {
			this.destroy();
			showMessage('You lost an account!')
		}
	}
...
function init() {
  ...
  // Setup the Menu for buying stuff
	var menu = new Menu('Data Farm', [
		new BuyButton('Buy Account', Wheat),
